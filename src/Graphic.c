...
static void draw_gem(int center_x, int center_y, int size, MLV_Color color) {
    int vx[6], vy[6];
    for (int i = 0; i < 6; i++) {
        vx[i] = (int) (center_x + size * cos(i * PI / 3));
        vy[i] = (int) (center_y + size * sin(i * PI / 3));
    }
...
static void draw_bar(int x, int y, int width, int height, double filled,
    ...
    MLV_draw_filled_rectangle((int) (x + width * filled), y, (int) (width * (1.0 - filled)),
...
static void draw_monster(Monster monster) {
    ...
    draw_bar((int) (x - radius * 1.5), (int) (y - radius * 1.5), radius * 3, radius / 2,
