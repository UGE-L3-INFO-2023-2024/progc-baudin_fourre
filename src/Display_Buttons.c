...
void draw_top_buttons(WindowInfo *win) {
    ...
    win->increase_mana_level =
        new_square((MAP_WIDTH * win->cell_size) * 4 / 5, win->cell_size * 1 / 4,
                   win->cell_size * 1 / 2);
...
void draw_inventory(Inventory inventory, WindowInfo *win) {
    Square s_gem;
    MLV_draw_text_box_with_font(
        MAP_WIDTH * win->cell_size + 1, MAP_HEIGHT * win->cell_size * 3 / 10,
        RIGHT_BAR_COLS * win->cell_size, MAP_HEIGHT * win->cell_size * 1 / 30,
        "Inventory:", win->right_bar_font, 0, TRANSPARANT, MLV_COLOR_BLACK,
